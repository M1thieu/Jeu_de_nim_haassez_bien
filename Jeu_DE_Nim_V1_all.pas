...
DEBUT
	...
	J1 <-- ""
	J2 <-- ""
	Bienvenue dans le jeu de nim, voici les regles : Il y a 21 allumettes , le but du jeu est de retirer 1,2 ou 3 allumettes chaque tours mais attention... Celui qui retire la dernière allumette perd la partie.');
	LIRE;
	...
	TANTQUE restant > 1
		...
		SI restant <= 1 ALORS // test de la condition de victoire du J2
			ECRIRE "Désolé ",J1," mais cette fois-ci vous perdez ",J2," c''est une victoire."
		SINON 
		...
		SI restant <= 1 ALORS // test de la condition de victoire du J1
			ECRIRE "Désolé ",J2," mais cette fois-ci vous perdez ",J1," c''est une victoire."
		SINON 
			ECRIRE "Il reste ",restant," allumettes."
		FINSI
...
BEGIN
	...
	J2 := '';
	FIN := 0 ;
	Writeln('Bienvenue dans le jeu de nim, voici les regles :');
	Writeln(' Il y a 21 allumettes , le but du jeu est de retirer ');
	Writeln('1,2 ou 3 allumettes chaque tours mais attention...');
	Writeln('Celui qui retire la dernière allumette perd la partie.');
	readln();
			...
			IF FIN = 0 THEN
				BEGIN
					...
					IF (restant<=1) THEN//test de la condition de victoire du joueur 1 
						BEGIN
							Writeln('Desole ',J2,' mais cette fois-ci vous perdez ',J1,' c''est une victoire.');
							FIN:=1;
						END
					ELSE
						BEGIN
			...
			IF FIN =0 THEN
				BEGIN	
					...
					IF (restant<=1) THEN//test de la condition de victoire du joueur 2
						BEGIN
							Writeln('Desole ',J1,' mais cette fois-ci vous perdez ',J2,' c''est une victoire.');
							FIN:= 1;
						END
					ELSE
						BEGIN
